...
page_top("Professor Responds to Multiple Classes");

if (!$submit)
{
  $CID = array();
  
  $result0 = db_exec("SELECT classid FROM classes WHERE professorid='$professorID' AND semester='$sem' AND year='$year'", $db, __FILE__, __LINE__);
  ...
  $total_students = 0;
  $questionperiodid = $list_of_classes = $temp_str = "";
  ...
  $result6 = db_exec("SELECT answersetid,responses FROM answersets WHERE ($temp_str) and questionsetid='1' and questionperiodid='$questionperiodid'",$db,__FILE__,__LINE__);
...
<?  if ($mode != "admin") { ?>
Welcome, Professor <?=$professor_name?>
<p>You are responding to the survey results for: <br>
<?  } else { ?>
You're viewing Professor <?=$professor_fullname?>'s responses to the survey results for: <br>
<?  } ?>
  
<?=$list_of_classes?></font></strong><p>
...
<? if ($mode == "prof") { ?>
If you are having problems with this form (i.e. your browser does not support Javascript), please use the 
<a href="multiclasses.php?classID=<?=$classID?>&mode=prof-compatible">non-Javascript page</a>.
<? } ?>
  
<br><center>
  
<? if ($mode != "admin") { ?>
<form method="post" action="multiclasses.php">
<? } ?>
...
<?  
  ...
  $result = db_exec("SELECT displayname FROM questionsets WHERE questionsetid = '1' LIMIT 0,1", $db, __FILE__, __LINE__);
...
function generateSetOfResults($qsID, $asID)
{
  ...
  while ($myrow = mysql_fetch_array($result)) 
  {
    for ($num = 1; $num <= 10; $num++)
    {
      ...
      $D = $theQuestion . "D";
      $E = $theQuestion . "E";
      if ($myrow[$theQuestion] != "") generateResult($myrow[$theQuestion],$theQuestion,$asID,$theQuestion ."-". $asID);
    } //for
  } //while
...
{
  global $db, $responses, $mode, $professorID, $sem, $year;
...
// generate RESPOND?
  ...
  {
    $result = db_exec("SELECT * FROM responses WHERE answersetid = '$asID' LIMIT 1", $db, __FILE__, __LINE__);
