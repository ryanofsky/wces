...
function listclasses()
{
  ...
  WHERE e.userid = '$userid' AND cl.classid = '8524'
  GROUP BY cl.classid
  ...
  while ($class = mysql_fetch_array($classes))
  {
    ...
    extract($class);
    if ($surveyed)  
      print ("  <LI><A HREF=\"campbell.php?classid=$classid\">" . ucfirst($semester) . " $year  - $scode$code <i>$name</i> (section $section)</a></LI>\n");
    else
...
function getquestionsets($classid)
{
  ...
  LEFT JOIN answersets AS a ON a.questionsetid = q.questionsetid AND a.classid = e.classid AND a.questionperiodid = '$questionperiodid'
  LEFT JOIN completesurveys AS cs ON cs.userid = e.userid AND cs.answersetid = a.answersetid
  WHERE e.userid = '$userid' AND e.classid = '8524' AND cs.userid IS NULL
  LIMIT 50",$db);
...
{
  print ('<form method=post action="campbell.php" id=form1 name=form1>');
