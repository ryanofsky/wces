 ...
 function listclasses()
  {
    ...
    while ($class = mysql_fetch_array($classes))
    {
      ...
      print ("  <LI>" . ucfirst($semester) . " $year  - $scode$code <i>$name$clname</i> (section $section) [ <A HREF=\"demo.php?editclass=$classid\">Edit</A> | <A HREF=\"demo.php?previewclass=$classid\">Preview</A> ]</LI>\n");
    }
    if ($first)
      print("None of your classes are being surveyed this semester.");
    else
      print("<li>All My Classes [ <a href=\"demo.php?editclass=all\">Edit</a> | <a href=\"demo.php?previewclass=all\">Preview</a> ]</li>\n");  
    print("</UL>\n");
  }
...
<tr><td colspan=2><input type=submit name=save value="Submit Changes"> <input type=submit name=cancel value="Cancel"><td></tr>
  ...
  function savequestions()
  {
    ...
    if ($delete)
      foreach($ABETQUESTIONS as $k => $v)
        if (isset($HTTP_GET_VARS["ABET$k"])) $delete = false;
  
    for($i = 1; $delete && $i <= 10; ++$i)
      if (isset($HTTP_GET_VARS["ratings$i"])) $delete = false;
  ...
  function previewquestions()
  {
    ...
    print("<h2>Preview of Questions for $classname (<a href=\"demo.php\">Back</a>)</h2>\n");
    ...
    if ($previewclass == "all")
    {
      ...
      if (count($questionsets) > 0)
      {
        ...
        print("<p><b>No custom question sets found</b></p>");
    }  
    else
    {
      print("<form method=post>\n");
      $s = new OldSurvey($db, $previewclass, false, false, "preview", "f", WIDGET_POST);
      $s->loadvalues();
      $s->display();
      print("</form>\n");
    ...
    print("<h2><a href=\"demo.php\">Back</a></h2>");
  }
